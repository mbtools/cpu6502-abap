CLASS zcl_cpu_00_speedrun DEFINITION
  PUBLIC
  CREATE PUBLIC.
...
CLASS zcl_cpu_00_speedrun IMPLEMENTATION.

  METHOD constructor.
    mo_bus = NEW zcl_cpu_00_bus_simple( ).
    IF iv_loadaddr > 0.
      mo_bus->zif_cpu_00_bus~load( iv_addr = iv_loadaddr iv_data = iv_rom ).
    ELSE.
      mo_bus->zif_cpu_00_bus~load( iv_addr = 0 iv_data = iv_rom ).
    ENDIF.
