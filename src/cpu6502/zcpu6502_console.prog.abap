...
CLASS lcl_console_controller DEFINITION FINAL.
  ...
  PRIVATE SECTION.
    DATA: mo_bus       TYPE REF TO zcl_cpu_00_bus_basic,
          mo_cpu       TYPE REF TO zcl_cpu_00_cpu,
...
AT SELECTION-SCREEN OUTPUT.
  ...
  DATA(lo_loader) = NEW zcl_cpu_00_rom_loader_smw0( iv_pattern = '*.BIN' ).
  DATA(lt_roms) = lo_loader->zif_cpu_00_rom_loader~list_roms( ).
...
CLASS lcl_console_controller IMPLEMENTATION.
  ...
  METHOD start_emulator.
    ...
    TRY.
        ...
        mo_bus->zif_cpu_00_bus~load( iv_addr = c_load_addr iv_data = mv_rom ).
  ...
  METHOD execute_step.
    DATA lv_log TYPE string.
    DATA ls_status TYPE zcl_cpu_00_cpu=>ts_status.
...
START-OF-SELECTION.
  ...
  IF r_smw0 = abap_true.
    IF p_rom IS INITIAL.
      ...
      RETURN.
    ENDIF.
    DATA(lo_smw0_loader) = NEW zcl_cpu_00_rom_loader_smw0( iv_pattern = '*.BIN' ).
    gv_rom = lo_smw0_loader->zif_cpu_00_rom_loader~load( CONV #( p_rom ) ).
  ELSE.
    IF p_path IS INITIAL.
      MESSAGE 'Please specify a ROM file path' TYPE 'S' DISPLAY LIKE 'E'.
      RETURN.
    ENDIF.
    DATA(lo_file_loader) = NEW zcl_cpu_00_rom_loader_file( ).
    gv_rom = lo_file_loader->zif_cpu_00_rom_loader~load( p_path ).
  ENDIF.
